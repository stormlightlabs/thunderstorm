...
| Pi          | `--provider` and `--model` on the pane's command           | `--thinking` on the same command                 | one session per multiplexer tab      | the `command` and `events.jsonl` a dispatch writes |
