...
> Copied from `stormlightlabs/thunderus` on 2026-09-19, when the workflow moved
> to this repository. The identifier above is unchanged, because open issues
> cite it; thunderus holds a document with the same identifier until it installs
> the plugin instead of carrying its own copy.
>
> **Statuses**, **Commands**, and the label set below describe the board as
> thunderus ran it. `trps` has since moved status onto a Projects V2
> single-select with three options and dropped the `status:*` and `blocked:*`
> labels entirely. Reconciling this document with that is
> stormlightlabs/thunderstorm#4.
