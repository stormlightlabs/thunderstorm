...
holds both, and `{{PLUGIN}}/scripts/check-commit-message.py --pr` reports them on
...
- Files under `internal/` need `name`, `last_updated`, and a ULID `id` in
  frontmatter. Generate identifiers with `{{PLUGIN}}/scripts/ulid.py`, and check the
  tree with `{{PLUGIN}}/scripts/check-frontmatter.py <dir>`, naming the tree
