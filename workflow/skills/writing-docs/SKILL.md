...
holds both, and `{{ROOT}}/scripts/check-commit-message.py --pr` reports them on
...
- Files under `internal/` need `name`, `last_updated`, and a ULID `id` in
  frontmatter. Generate identifiers with `{{ROOT}}/scripts/ulid.py`, and check the
  tree with `{{ROOT}}/scripts/check-frontmatter.py`.
