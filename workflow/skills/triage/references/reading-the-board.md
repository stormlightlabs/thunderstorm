...
The 24-hour rule needs the moment the claim happened. `updated_at` is not that
moment: any comment, assignee change, or label bumps it, so a claim abandoned a
week ago reads fresh the moment somebody comments on it. Nor does the issue
carry the board write, because Status lives on the project and its history is
not in the issue timeline.

The assignment is in the timeline, and a claim assigns. Read that instead:

```sh
gh api "repos/<owner>/<repo>/issues/<n>/timeline" \
  --jq '[.[] | select(.event=="assigned")] | last'
```

Read the open pull requests once for which claims have one. Both are per-issue
calls, so make them only for issues the board shows as `In Progress`. Where the
timeline is unavailable, report the age as unverified rather than passing
`updated_at` off as an answer.
