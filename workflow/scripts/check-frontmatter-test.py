...
# internal/ is written by the first spec or idea, so a bare run has no tree to
# read until then and says so rather than passing on nothing.
missing_default = subprocess.run(
    [sys.executable, str(TOOL), str(ROOT / "nothing-here")],
    capture_output=True,
    text=True,
    check=False,
)
check(
    ...
    missing_default.returncode == 2 and "is not a directory" in missing_default.stderr,
    f"exit={missing_default.returncode} stderr={missing_default.stderr.strip()[:120]}",
)
...
MS = "https://github.com/stormlightlabs/trps/milestone/1"
