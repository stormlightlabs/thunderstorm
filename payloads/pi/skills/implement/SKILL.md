...
`${THUNDERSTORM_PLUGIN_ROOT}/scripts/check-commit-message.py --pr` reports both before the merge.

Push with `${THUNDERSTORM_PLUGIN_ROOT}/scripts/push-verified.sh`, which compares the remote ref to
