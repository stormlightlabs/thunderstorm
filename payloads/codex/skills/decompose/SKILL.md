...
one run. A spec is decomposed after it merges to `main`, so the
...
filing anyway produces an issue that a worker will hand straight back.
...
One unit of work each, added to the board as `Todo`, under `github-board`'s
**File new work**. What a change is, where it lands, and what it risks belong
in the body, where a reader gets them in prose rather than off a label.
...
| Sub-issue | Owns |
| --- | --- |
| #76 detect repeated metaphor terms | `crates/core/src/detector/repetition.rs` |
| #79 add the `--format json` flag | `crates/cli/src/main.rs` |
```
