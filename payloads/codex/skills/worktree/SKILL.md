...
branch, and its own branch never leaves `origin/main`. Git refuses one branch in
...
`.codex/.gitignore` keeps a worktree that lands at `.codex/worktrees/` out of
...
A worktree whose issue is no longer `In Progress` is stale. Remove it after
