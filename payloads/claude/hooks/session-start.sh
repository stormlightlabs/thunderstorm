...
#
# A web container begins with no ~/.cargo/registry and no target/, so the check
# gate would otherwise download every dependency before the first command a
# reviewer asks for. Local checkouts already have these, so this exits early
# outside the cloud.
set -uo pipefail
...
# there on lints the older compiler does not have yet, so tracking stable costs
# a few seconds and removes the gap.
# Updating stable is not selecting it: an image whose default is a
# version-named toolchain would download a compiler nothing runs.
# --no-self-update keeps rustup itself out of the critical path.
...
# Every step reports and carries on. A container that cannot reach the registry
# should cost the warm, not the session.
#
