...
- Quoted source under `meta/` is not house prose. `meta/tropes.md` and the
  corpus in `meta/examples/` stay as they are; the slop examples are sloppy on
  purpose and the tests read them.
