...
`${CLAUDE_PLUGIN_ROOT}/agents/adversarial-reviewer.md` lists what it hunts and what it owes
...
- Does it add a trait, a layer, or a configuration point that one concrete
  helper would cover? A trait earns its place at a real boundary, not at the
  first place two things look alike.
...
2. The conventions the surrounding code already keeps: module order, error
   policy, trait boundaries, and a documented invariant behind every `unwrap`,
   `expect`, or `panic!`.
...
A diff touching `internal/`, `meta/`, `README.md`, or `.claude/` is a change
...
anyone goes back to it. The model and level because the pass that
ran is chosen at dispatch, and this line is the only record of it afterwards. Use the identifier the harness reports, not a friendly name.
