...
`${CLAUDE_PLUGIN_ROOT}/scripts/check-commit-message.py --pr --title-file <f> --body-file <f>`
...
`${CLAUDE_PLUGIN_ROOT}/scripts/check-commit-message.py` checks the shape: the type, the
