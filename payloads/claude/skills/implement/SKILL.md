...
`${CLAUDE_PLUGIN_ROOT}/scripts/check-commit-message.py --pr` reports both before the merge.

Push with `${CLAUDE_PLUGIN_ROOT}/scripts/push-verified.sh`, which compares the remote ref to
